...
interface BreakoutMultiProps {
  familyId: string;
  userId: string;
  userName: string;
  onSaveScore: (score: number, level: number) => void;
}

type MultiState = "menu" | "waiting" | "playing" | "finished";

export default function BreakoutMulti({ familyId, userId, userName, onSaveScore }: BreakoutMultiProps) {
  ...
  const setupBroadcast = useCallback((sessionId: string) => {
    ...
    const channel = supabase
      .channel(`breakout-play-${sessionId}`)
  ...
  const createSession = useCallback(async () => {
    ...
    if (data) {
      ...
      const channel = supabase
        .channel(`breakout-session-${data.id}`)
        .on("postgres_changes", {
          event: "UPDATE",
          schema: "public",
          table: "game_sessions",
  ...
  }, [joinCode, familyId, userId, loadOpponentName, setupBroadcast]);
    ...
    return (
      <div className="text-center py-8">
        ...
        <p className="text-xs mb-4" style={{ color: "var(--dim)" }}>
          Demande à un membre de ta famille de rejoindre depuis l&apos;onglet Breakout Multi
        </p>
  ...
  return (
    <div>
      ...
      <BreakoutGame onGameOver={handleGameOver} />
    </div>
  );
}
